// Vercel serverless function: /api/learning
// Handles GET (list), POST (create), PUT (update), DELETE (delete)
// Uses GitHub API to update data/learning.json

const REPO_OWNER = process.env.GITHUB_REPO_OWNER;
const REPO_NAME = process.env.GITHUB_REPO_NAME;
const GITHUB_TOKEN = process.env.GITHUB_TOKEN;
const DATA_PATH = 'data/learning.json';
...
async function saveData(items, sha) {
  ...
  const resp = await fetch(url, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${GITHUB_TOKEN}`, Accept: 'application/vnd.github.v3+json' },
    body: JSON.stringify({ message: 'Update learning.json via admin', content: content, sha: sha })
  });
  return resp.ok;
}
